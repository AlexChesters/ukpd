...
- Use Node.js 22 or 24 and npm 10, matching `package.json` and CI.
...
The Jest tests and examples make live requests to `https://data.police.uk`, so failures can reflect API availability or network conditions. CI runs `npm ci`, `npm run build`, and `npm test` on Node 22 and Node 24.
